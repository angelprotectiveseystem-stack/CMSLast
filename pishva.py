...
from config import (PISHVA_ID, STATUS_NORMAL, STATUS_BAD, STATUS_DANGER, STATUS_APS,
                    ...
                    ST_GROUP_ID, ST_UPDATE_VERSION, ST_UPDATE_DESC,
                    NEW_YEAR_PASSWORD)
import io
...
async def pishva_backup(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"{box('💾 دریافت بکاپ')}\n\n📌 بازه زمانی را انتخاب کنید:",
        reply_markup=kb.kb_backup_period(),
        parse_mode="Markdown"
    )
...
async def backup_format_select(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer("⏳ در حال تهیه بکاپ...", show_alert=False)
    fmt = query.data.split("_")[-1]
    period = ctx.user_data.get("backup_period", "all")

    try:
        from backup_utils import generate_excel_backup, generate_word_backup
        ts = now_shamsi()
        if fmt == "excel":
            buf = await generate_excel_backup(period)
            filename = f"chess_backup_{period}.xlsx"
            await ctx.bot.send_document(
                chat_id=PISHVA_ID,
                document=buf,
                filename=filename,
                caption=f"💾 بکاپ {period} — {ts}"
            )
        else:
            buf = await generate_word_backup(period)
            filename = f"chess_backup_{period}.docx"
            await ctx.bot.send_document(
                chat_id=PISHVA_ID,
                document=buf,
                filename=filename,
...
async def pishva_repair(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.from_user.id != PISHVA_ID:
        ...
        return
    await query.answer()
    repair_on = await db.get_setting("repair_mode", "0")
    status = "🔧 فعال" if repair_on == "1" else "✅ غیرفعال"
    await query.edit_message_text(
        f"{box('🔧 حالت تعمیر')}\n\nوضعیت فعلی: {status}",
...
async def pishva_vault(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Players who have at least one match
    rows = []
