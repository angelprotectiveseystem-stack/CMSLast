...
# ─── پنل امنیتی APS: صف انتظار و بلاک ──────────────────────────
def kb_security_panel():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⏳ صف انتظار", callback_data="security_queue"),
         InlineKeyboardButton("🚫 لیست بلاک‌شده‌ها", callback_data="security_blocked")],
        [InlineKeyboardButton("🔙 بازگشت", callback_data="menu_pishva")],
    ])


def kb_queue_list(requests):
    rows = []
    for r in requests:
        name = r["full_name"] or r["username"] or str(r["telegram_id"])
        rows.append([InlineKeyboardButton(f"⏳ {name}", callback_data=f"queueview_{r['id']}")])
...
def kb_blocked_list(blocked):
    rows = []
    for b in blocked:
        name = b["full_name"] or b["username"] or str(b["telegram_id"])
        rows.append([InlineKeyboardButton(f"🚫 {name}", callback_data=f"blockedview_{b['telegram_id']}")])
    rows.append([InlineKeyboardButton("🔙 بازگشت", callback_data="security_panel")])
    return InlineKeyboardMarkup(rows)


def kb_blocked_item_actions(telegram_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔓 آنبلاک", callback_data=f"unblock_{telegram_id}")],
        [InlineKeyboardButton("🔙 بازگشت", callback_data="security_blocked")],
    ])


def kb_block_confirm(target_token, back_cb="security_panel"):
    """target_token: either 'req_<id>' (block from an access-request) or a raw telegram_id (block directly)"""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ بله، بلاک کن", callback_data=f"blockconfirm_{target_token}"),
