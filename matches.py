...
from helpers import (box, separator, now_shamsi, today_gregorian, today_shamsi,
                     notify_pishva, broadcast_to_admins, check_status_gate,
                     smart_lottery, warning_bar_player)
from config import (PISHVA_ID, ST_MATCH_WHITE, ST_MATCH_BLACK, ST_MATCH_DATE,
                    ST_MATCH_RESULT, ST_MATCH_DRAW_REASON, ST_MATCH_ELIMINATE,
                    ST_SEARCH_MATCH, ST_LOTTERY_SCOPE, ST_LOTTERY_CLASS)


# ─── Add Match ────────────────────────────────────────────────
async def match_add_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if await check_status_gate(query, "match_registration_enabled"):
        return
    ...
    if len(players) < 2:
        await query.edit_message_text(
            f"❌ *خطا — کد ۰۰۱*\n\n"
            f"حداقل دو بازیکن ادامه‌دهنده برای شروع مسابقه وجود ندارد.",
...
async def match_black_selected(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"_(فرمت: YYYY-MM-DD مثلاً {today_g})_",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(f"📅 انتخاب تاریخ امروز ({today})", callback_data=f"mdate_today")]
        ]),
        parse_mode="Markdown"
    )
...
async def match_date_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["match_date"] = date_str
    return await _finalize_match(update, ctx, via_query=False)


async def _finalize_match(update: Update, ctx: ContextTypes.DEFAULT_TYPE, via_query: bool):
    white_id = ctx.user_data["match_white"]
    black_id = ctx.user_data["match_black"]
    date = ctx.user_data["match_date"]
    ...
    await db.log_action(uid, "create_match", f"ثبت مسابقه: {wp['full_name']} vs {bp['full_name']}", mid)
    ...
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🏆 ثبت نتیجه", callback_data="match_result"),
         InlineKeyboardButton("✅ تأیید و بازگشت", callback_data="back_matches")],
    ])
    if via_query:
...
async def match_result_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for m in pending[:15]:
        rows.append([InlineKeyboardButton(
            f"⬜{m['white_name']} ⚔️ {m['black_name']}⬛ | {m['match_date'] or '?'}",
...
async def result_white(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await db.log_action(query.from_user.id, "match_result", f"نتیجه مسابقه {mid}: برد سفید", mid)

    # Check if black (loser) is elite/special
    ...
    await query.edit_message_text(
        f"🥇 برد سفید (*{m['white_name']}*) ثبت شد.{warn_text}\n\n"
        f"آیا می‌خواهید *{m['black_name']}* (بازنده) حذف شود؟",
...
async def result_black(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await db.log_action(query.from_user.id, "match_result", f"نتیجه مسابقه {mid}: برد سیاه", mid)
    ...
    await query.edit_message_text(
        f"🥇 برد سیاه (*{m['black_name']}*) ثبت شد.{warn_text}\n\n"
        f"آیا می‌خواهید *{m['white_name']}* (بازنده) حذف شود؟",
...
async def draw_reason(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    reason_map = {
        "pat": "پات", "time": "اتمام زمان",
        "moves": "حرکات بسیار", "repeat": "سه حرکت تکراری"
    }
    if reason_key == "other":
    ...
    await _save_draw(query, ctx, mid, reason)
...
async def draw_reason_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if mid:
        await _save_draw_msg(update, ctx, mid, reason)
    return ConversationHandler.END


async def _save_draw(query, ctx, mid: int, reason: str):
    m = await db.get_match(mid)
    ...
    await db.log_action(query.from_user.id, "match_result", f"نتیجه مسابقه {mid}: تساوی ({reason})", mid)
    await query.edit_message_text(
        f"🤝 تساوی ثبت شد.\n📋 علت: {reason}",
        reply_markup=kb.kb_back("matches"),
        parse_mode="Markdown"
...
async def _save_draw_msg(update, ctx, mid: int, reason: str):
    m = await db.get_match(mid)
    await db.set_match_result(mid, "draw", reason, update.effective_user.id)
    await db.update_player_stats(m["white_player_id"], "draw")
    await db.update_player_stats(m["black_player_id"], "draw")
    await db.log_action(update.effective_user.id, "match_result", f"نتیجه مسابقه {mid}: تساوی ({reason})", mid)
...
async def eliminate_yes(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await db.log_action(query.from_user.id, "eliminate_player", f"حذف بازیکن از مسابقه: {p['full_name']}", pid)
    await query.edit_message_text(
...
async def eliminate_no(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    p = await db.get_player(pid)
    await query.edit_message_text(
        f"✅ *{p['full_name']}* در لیست ادامه‌دهندگان باقی ماند.",
        reply_markup=kb.kb_back("matches"),
        parse_mode="Markdown"
    )
...
async def match_hist_filter(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not matches:
        await query.edit_message_text(
            f"❗ هیچ مسابقه‌ای در این بازه یافت نشد.",
            reply_markup=kb.kb_match_history_filter(),
            parse_mode="Markdown"
        )
        return
    label = {"today": "امروز", "week": "این هفته", "month": "این ماه", "all": "کل مسابقات"}.get(period, period)
    await query.edit_message_text(
        f"{box(f'📋 مسابقات — {label}')}\n\n📌 یک مسابقه انتخاب کنید:",
        reply_markup=kb.kb_match_list(matches),
        parse_mode="Markdown"
    )
...
async def match_hist_search_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("🔍 نام بازیکن، تاریخ یا کلاس را وارد کنید:")
...
async def match_view(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    result_map = {"white": f"🥇 برد {m['white_name']}", "black": f"🥇 برد {m['black_name']}",
    ...
    if m["created_by"]:
        if m["created_by"] == PISHVA_ID:
            admin = await db.get_setting("pishva_display_name", "پیشوا")
        else:
            a = await db.get_admin(m["created_by"])
            admin = a["display_name"] or a["full_name"] if a else str(m["created_by"])

    text = (
        f"{box('♟️ جزئیات مسابقه')}\n\n"
        f"⬜ سفید: *{m['white_name']}* [{m.get('white_class','') or ''}]\n"
        f"⬛ سیاه: *{m['black_name']}* [{m.get('black_class','') or ''}]\n"
        f"🏆 نتیجه: {result_map.get(m['result'], '—')}\n"
        f"📋 علت تساوی: {m['draw_reason'] or '—'}\n"
        f"📅 تاریخ: `{m['match_date'] or '—'}`\n"
        f"👤 ثبت‌کننده: {admin or '—'}\n"
        f"⏱️ ثبت در: `{str(m['created_at'] or '')[:19]}`\n"
...
async def match_panel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Last edit
    edited = [m for m in matches if m["updated_at"]]
    last_edit = "—"
    if edited:
        last = max(edited, key=lambda m: m["updated_at"])
        last_edit = f"`{str(last['updated_at'])[:19]}`"

    text = (
        f"{box('📊 پنل مدیریت مسابقات')}\n\n"
        f"✏️ آخرین ویرایش: {last_edit}\n"
        f"🏅 تورنمنت فعال: *{default_t['name'] if default_t else 'ندارد'}*\n\n"
        f"{separator('📈 آمار کلی')}\n"
        f"♟️ کل مسابقات: `{len(matches)}`\n"
        f"✅ با نتیجه: `{sum(1 for m in matches if m['result'])}`\n"
        f"⏳ بدون نتیجه: `{sum(1 for m in matches if not m['result'])}`\n"
        f"👥 کل بازیکنان: `{len(players)}`\n\n"
...
async def lottery_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.edit_message_text(
        f"{box('🎲 سیستم قرعه‌کشی هوشمند')}\n\n📌 انتخاب محدوده بازیکنان:",
        reply_markup=kb.kb_lottery_scope(),
        parse_mode="Markdown"
    )
...
async def _run_lottery(query, ctx, players):
    if len(players) < 2:
        await query.edit_message_text(
            "❌ تعداد بازیکنان ادامه‌دهنده در این محدوده کمتر از ۲ نفر است.",
            reply_markup=kb.kb_back("matches")
        )
        return
    ...
    warn = ""
    if all_played:
        warn = "\n\n⚠️ همه بازیکنان موجود قبلاً با هم مسابقه داده‌اند. قرعه‌کشی تصادفی انجام شد."

    text = (
        f"╼╼╼╼╼╼ 🎲 نتیجه قرعه‌کشی ╾╾╾╾╾╾\n\n"
        f"⬜ سفید:  *{p1['full_name']}* — کلاس [{p1.get('class_name','')}]\n"
        f"⬛ سیاه:  *{p2['full_name']}* — کلاس [{p2.get('class_name','')}]\n\n"
    ...
    await query.edit_message_text(
        text,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ تأیید و ثبت مسابقه", callback_data=f"lottery_confirm_{p1['id']}_{p2['id']}")],
...
async def lottery_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    today_g = today_gregorian()
    await query.edit_message_text(
        f"📅 تاریخ مسابقه را وارد کنید:\n_(فرمت: YYYY-MM-DD مثلاً {today_g})_",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(f"📅 انتخاب تاریخ امروز ({today_shamsi()})", callback_data="mdate_today")]
        ]),
        parse_mode="Markdown"
    )
    return ST_MATCH_DATE
