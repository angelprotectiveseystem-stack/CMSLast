...
from config import (
    ...
    ST_ADMIN_ROLE_SELECT, ST_CLASS_NAME, ST_PLAYER_CLASS_SELECT, ST_PLAYER_NAME,
    ST_TOURNAMENT_NAME, ST_TOURNAMENT_EDIT, ST_MATCH_WHITE, ST_MATCH_BLACK,
    ST_MATCH_DATE, ST_MATCH_RESULT, ST_MATCH_DRAW_REASON, ST_MATCH_ELIMINATE,
    ...
    ST_FEEDBACK_TEXT, ST_SUGGESTION_TEXT, ST_FEATURE_TITLE, ST_FEATURE_DESC,
    ST_PRAISE_TEXT, ST_PISHVA_NAME_CHANGE, ST_ADMIN_NAME_CHANGE,
...
from pishva import (
    ...
    pishva_group, group_id_save, pishva_vault
)
from comms import (
...
from misc import (
    ...
    admin_kick, admin_msg_start, admin_task_start, admin_profile,
...
def build_application():
    ...
    auth_conv = ConversationHandler(
        ...
        states={
            ST_ROLE_SELECT: [CallbackQueryHandler(on_role_select, pattern="^role_")],
            ST_PISHVA_PASSWORD: [MessageHandler(filters.TEXT & ~filters.COMMAND, on_pishva_password)],
            ST_ADMIN_USERNAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, on_admin_username)],
            ST_ADMIN_FULLNAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, on_admin_fullname)],
    ...
    # ─── Tournament ConversationHandler ───────────────────────
    tourn_conv = ConversationHandler(
        ...
        states={
            ST_TOURNAMENT_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, tourn_add_name)],
            ST_TOURNAMENT_EDIT: [MessageHandler(filters.TEXT & ~filters.COMMAND, tourn_edit_name)],
        },
        fallbacks=[CallbackQueryHandler(back_tournament, pattern="^back_tournament$")],
        per_message=False,
    )

    # ─── Player ConversationHandler ───────────────────────────
    player_conv = ConversationHandler(
        ...
        states={
            ST_CLASS_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, class_add_name)],
            ST_PLAYER_CLASS_SELECT: [CallbackQueryHandler(player_class_selected, pattern="^pclass_")],
            ST_PLAYER_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, player_add_name)],
            ST_WARNING_REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, player_warn_reason)],
            ST_NOTE_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, player_note_save)],
            ST_EDIT_PLAYER_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, player_editname_save)],
            ST_SEARCH_PLAYER: [MessageHandler(filters.TEXT & ~filters.COMMAND, player_search_run)],
        },
    ...
    match_conv = ConversationHandler(
        ...
        states={
            ST_MATCH_WHITE: [CallbackQueryHandler(match_white_selected, pattern="^mwhite_")],
            ST_MATCH_BLACK: [CallbackQueryHandler(match_black_selected, pattern="^mblack_")],
            ST_MATCH_DATE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, match_date_text),
                CallbackQueryHandler(match_date_today, pattern="^mdate_today$"),
            ],
            ST_MATCH_DRAW_REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, draw_reason_text)],
            ST_SEARCH_MATCH: [MessageHandler(filters.TEXT & ~filters.COMMAND, match_hist_search_run)],
        },
        fallbacks=[CallbackQueryHandler(back_matches, pattern="^back_matches$")],
        per_message=False,
    )

    # ─── Comms ConversationHandler ────────────────────────────
    comms_conv = ConversationHandler(
        ...
        states={
            ST_SEND_MSG_SELECT_ADMIN: [CallbackQueryHandler(comms_msg_target, pattern="^cmsg_")],
            ST_SEND_MSG_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, comms_msg_send)],
            ST_ANNOUNCEMENT_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, comms_announce_text)],
            ST_ANNOUNCEMENT_FILE: [
                ...
                MessageHandler(filters.PHOTO | filters.Document.ALL | filters.VIDEO | filters.AUDIO | filters.VOICE,
                               comms_announce_file_received),
            ],
            ST_NEWS_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, comms_news_send)],
        },
        fallbacks=[CallbackQueryHandler(menu_comms, pattern="^back_comms$")],
        per_message=False,
    )
    ...
    task_conv = ConversationHandler(
        ...
        states={
            ST_TASK_SELECT_ADMIN: [CallbackQueryHandler(task_to_admin, pattern="^task_to_")],
            ST_TASK_TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND, task_title_received)],
            ST_TASK_DESC: [MessageHandler(filters.TEXT & ~filters.COMMAND, task_desc_received)],
            ST_TASK_DONE_REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, task_fail_reason)],
        },
    ...
    feedback_conv = ConversationHandler(
        ...
        states={
            ST_FEEDBACK_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, fb_text_received)],
            ST_FEATURE_DESC: [MessageHandler(filters.TEXT & ~filters.COMMAND, fb_feature_desc)],
        },
        fallbacks=[CallbackQueryHandler(menu_feedback, pattern="^menu_feedback$")],
        per_message=False,
    )

    # ─── Pishva ConversationHandler ───────────────────────────
    pishva_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(identity_pishva_start, pattern="^identity_pishva$"),
            CallbackQueryHandler(identity_admin_start, pattern="^identity_admin$"),
        ...
        states={
            ST_PISHVA_NAME_CHANGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, identity_pishva_save)],
            ST_ADMIN_NAME_CHANGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, identity_admin_save)],
            ST_NEW_YEAR_PASSWORD: [MessageHandler(filters.TEXT & ~filters.COMMAND, newyear_password)],
            ST_REPAIR_REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, repair_reason_save)],
            ST_GROUP_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, group_id_save)],
            ST_UPDATE_VERSION: [MessageHandler(filters.TEXT & ~filters.COMMAND, update_version_received)],
            ST_UPDATE_DESC: [MessageHandler(filters.TEXT & ~filters.COMMAND, update_desc_received)],
            ST_ADMIN_WARNING_REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_warn_reason)],
            ST_SEND_MSG_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, comms_msg_send)],
        },
    ...
    # ─── Team ConversationHandler ─────────────────────────────
    team_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(teams_add_start, pattern="^teams_add$")],
        states={
            ST_TEAM_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, team_name_received)],
            ST_TEAM_SLOGAN: [
            ...
            ST_TEAM_REQUESTER: [MessageHandler(filters.TEXT & ~filters.COMMAND, team_requester_received)],
        },
        fallbacks=[CallbackQueryHandler(teams_list, pattern="^teams_list$")],
        per_message=False,
    )

    # ─── Add all ConversationHandlers ─────────────────────────
    ...
    app.add_handler(CallbackQueryHandler(back_class_manage, pattern="^back_class_manage$"))
    ...
    # Access requests (global — outside any conversation)
    ...
    # Help
    app.add_handler(CallbackQueryHandler(help_section, pattern="^help_"))
