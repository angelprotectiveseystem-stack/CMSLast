...
def _disabled_page():
    html = (
        ...
        "<div>⚠️ این پنل به علت مشکل داده غیرفعال گشته و یا اتصال به سرور ممکن نیست.<br>"
        "صفحه را رفرش کنید و در صورت ادامه مشکل با محمد پارسا کریمی در ارتباط باشید🙏</div></body></html>"
    )
...
async def panel_login(request):
    try:
        ...
        body = {}
    if not await _panel_enabled():
        return _json({"ok": False, "error": "این پنل به علت مشکل داده غیرفعال گشته و یا اتصال به سرور ممکن نیست. صفحه را رفرش کنید و در صورت ادامه مشکل با محمد پارسا کریمی در ارتباط باشید🙏"})
