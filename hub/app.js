...
if(tg){
  ...
  // تمام‌صفحه‌ی واقعی (مثل ویب‌چس): هدر تلگرام جمع می‌شود و دکمه‌های بالا
  // کوچک می‌شوند. requestFullscreen فقط در Bot API 8.0+ هست، پس با
  // isVersionAtLeast چک می‌کنیم تا روی کلاینت‌های قدیمی خطا ندهد (expand()
  // بالا به‌عنوان فallback همیشه اجرا شده).
  try{
    if(tg.isVersionAtLeast && tg.isVersionAtLeast("8.0") && typeof tg.requestFullscreen === "function"){
      tg.requestFullscreen();
    }
  }catch(e){}
  // خودِ تلگرام موقع تمام‌صفحه یا expand، متغیرِ CSS
  // --tg-content-safe-area-inset-top را روی صفحه ست می‌کند (نیازی به JS
  // اضافه نیست)؛ در style.css همین متغیر برای پدینگِ بالای topbar استفاده شده.
}
