...
async def announce_match_result(bot, match_id: int, result: str,
    ...
    """ارسال نتیجه مسابقه به گروه اعلانات و کانال اعلانات (هرکدام که تنظیم شده باشد)"""
    broadcast_on = await db.get_setting("broadcast_result_enabled", "1")
    if broadcast_on != "1":
        return
    ...
    for tid in filter(None, [group_id, channel_id]):
        try:
...
async def weekly_champion_job(context):
    ...
    broadcast_on = await db.get_setting("broadcast_champion_enabled", "1")
    if broadcast_on == "1":
        for tid in filter(None, [group_id, channel_id]):
            try:
