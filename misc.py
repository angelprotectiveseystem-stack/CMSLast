...
async def perm_toggle(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await db.log_action(PISHVA_ID, "toggle_perm", f"{perm_name} -> {perms[perm_name]}", tid)

    # Notify admin
    status = "فعال ✅" if perms[perm_name] else "غیرفعال ❌"
    try:
        await ctx.bot.send_message(
            chat_id=tid,
            text=f"{'⬆️' if perms[perm_name] else '⬇️'} دسترسی *{perm_name}* برای شما {status} شد.\n⏱️ `{now_shamsi()}`",
            parse_mode="Markdown"
        )
    except Exception:
        pass
