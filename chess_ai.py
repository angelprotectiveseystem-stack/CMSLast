...
def search_moves(board: pychess.Board, depth: int):
    """جست‌وجوی سطح‌بالا برای *همه‌ی* حرکت‌های مجازِ موقعیتِ فعلی: برای هر
    حرکت، ارزشِ آن را (از دیدِ بازیکنِ نوبت‌دار در board، هرچه بزرگ‌تر
    بهتر) با negamax محاسبه می‌کند و دیکشنریِ {Move: value} برمی‌گرداند.
    برخلافِ choose_move که فقط بهترین حرکت را می‌خواهد، این تابع کل نقشه‌ی
    ارزیابی را برمی‌گرداند تا chess_analysis.py بتواند حرکتِ واقعاً بازی‌شده
    را با بهترین حرکتِ ممکن مقایسه کند (تحلیلِ پس از بازی). sync و
    CPU-bound — فراخوان باید با asyncio.to_thread اجرا کند.
    """
