...
# نکته (۲۰۲۶-۰۹، اصلاح‌شده): پیش‌فرض قبلی اینجا gemini-3.5-flash-lite بود — یه مدلِ
# نسل ۳ که اصلاً نمی‌تونه فکرکردن رو کامل خاموش کنه (حداقلش thinkingLevel=minimal
# است که بازم یه گذرِ فکریِ اجباری داره و thought signature لازمه). چون هر پیامِ
# دستیار معمولاً حداقل دو بار Gemini صدا می‌زنه (تصمیمِ «تابع صدا بزنم یا نه» + جوابِ
# نهایی بعد از نتیجه‌ی تابع)، و FALLBACK_MODELS هم قبلاً فقط شاملِ مدل‌های نسل ۳ بود
# (یعنی حتی اگه مدلِ اول جواب نمی‌داد، به یه مدلِ دیگه‌ی هم‌کند می‌رفت)، کلِ زنجیره
# همیشه رویِ مدل‌هایی بود که تفکرِ اجباری داشتن — همین چیزی بود که باعث می‌شد یه
# پیام تا چند دقیقه طول بکشه (به‌خصوص با تایم‌اوت/retry روی هاپ‌های پی‌درپی).
# gemini-2.5-flash-lite طبق مستنداتِ رسمیِ گوگل پیش‌فرضش «بدون فکرکردن» است (برخلافِ
# نسل ۳ که حداقلش minimal است) و شاتدأونِ رسمی‌اش هنوز اکتبر ۲۰۲۶ است — یعنی هنوز
# باید کار کنه؛ گزارش‌های پراکنده‌ی ۴۰۴ زودهنگام (تیرماه) رسمی و قطعی نبودن. اگه این
# مدل واقعاً از کار افتاده باشه، همون ۴۰۴ باعث می‌شه زنجیره‌ی زیر فوری (بدون تاخیرِ
# تایم‌اوت) بره سراغ مدلِ بعدی — پس گذاشتنش اول ریسکِ کمی داره ولی اگه زنده باشه
# سرعت رو کاملاً برمی‌گردونه. اگه بازم کند بود، لاگِ Railway رو برای کدِ HTTP واقعیِ
# هر مدل (نه فقط «کند بود») چک کن تا مشخص بشه کدوم مدل واقعاً داره جواب می‌ده.
